...
# 类型码 -> 统一 doc_type
FJ_TYPE_MAP = {
    101: "constitution", 102: "constitution", 110: "law", 120: "law",
    130: "judicial_interpretation", 140: "decision", 150: "admin_reg",
    155: "admin_reg", 160: "judicial_interpretation", 170: "judicial_interpretation",
    180: "judicial_interpretation", 190: "other", 195: "rule", 200: "rule",
}
...
class NPCAdapter:
    ...
    def search(self, keyword: str = "", page: int = 1, page_size: int = 20,
        ...
        d = _http_json(SEARCH_URL, "POST", body)
        if d.get("code") != 200:
    ...
    def get_detail(self, bbbs: str) -> dict:
        d = _http_json(f"{DETAIL_URL}?bbbs={bbbs}")
        if d.get("code") != 200:
    ...
    def download_docx(self, bbbs: str, title: str = "") -> bytes:
        d = _http_json(DOWNLOAD_URL, "POST", [{"bbbs": bbbs, "format": "docx"}])
    ...
    def to_record(self, jur_id: str, row: dict, detail: Optional[dict] = None,
        ...
        if docx_raw is not None:
            ...
            for no, body in split_sections(text):
